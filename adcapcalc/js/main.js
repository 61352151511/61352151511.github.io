...
advApp.controller('advController', ['$document', '$filter', '$scope', function($document, $filter, $scope) {
  ...
  function loadExportedJson(str) {
    ...
    for (k in planets) {
      if (obj.hasOwnProperty(planets[k])) {
        ...
        $scope[planets[k]].goldMultiplier = obj[planets[k]].goldMultiplier;
  ...
  function calcState(loc) {
    ...
    for (i = 0; i < loc.investments.length; i++) {
      ...
      if (loc.goldMultiplier > 0 || loc.bonusMultiplier > 0 || loc.suits[suitFromName('gold')][0] || loc.suits[suitFromName('blue')][0]) {
        loc.investments[i][3] *= (loc.goldMultiplier) + loc.bonusMultiplier + (loc.suits[suitFromName('gold')][0] ? $scope.suitList[suitFromName('gold')][1] : 0) + (loc.suits[suitFromName('blue')][0] ? $scope.suitList[suitFromName('blue')][1] : 0);
      }
  ...
  $scope.isEvent = function() {
    return $scope.ref == $scope.cashella;
  };
...
/*        if ($scope.selectAll[1]) {
  ...
  $scope.setWorld = function(planet) {
    ...
    if ($scope.isTab('Managers') && ($scope.isWorld('mars') || $scope.isEvent())) {
      $scope.setTab('Investments');
    }
  };
  ...
  function loadDefaults() {
    $scope.cashella.angelScale = 3;
    $scope.cashella.baseCost = [0, 0, 0, 0, 0, 0, 0, 0, 0];
    $scope.cashella.basePower = [0, 0, 0, 0, 0, 0, 0, 0, 0]; // Cost 1 / Cost 2
    $scope.cashella.baseProfit = [0, 0, 0, 0, 0, 0, 0, 0, 0];
    $scope.cashella.baseSpeed = [0, 0, 0, 0, 0, 0, 0, 0, 0];
    $scope.cashella.hasMegaTickets = false;
    $scope.cashella.investments = [
      ['Investment 1', 0, false, 0, 0, 0, 0],
      ['Investment 2', 0, false, 0, 0, 0, 0],
      ['Investment 3', 0, false, 0, 0, 0, 0],
      ['Investment 4', 0, false, 0, 0, 0, 0],
      ['Investment 5', 0, false, 0, 0, 0, 0],
      ['Investment 6', 0, false, 0, 0, 0, 0],
      ['Investment 7', 0, false, 0, 0, 0, 0],
      ['Investment 8', 0, false, 0, 0, 0, 0],
      ['Investment 9', 0, false, 0, 0, 0, 0]
    ];
    ...
    for (var p in planets) {
      ...
      $scope[planets[p]].goldMultiplier = 0;
  ...
  function loadUnlocks() {
    $scope.cashella.unlocks[0] = [[1, [1, 1]]]; // [Amount Owned, [Investment Speed/Investment Profit, Multiplier]]
    $scope.cashella.unlocks[1] = [[1, [1, 1]]];
    $scope.cashella.unlocks[2] = [[1, [1, 1]]];
    $scope.cashella.unlocks[3] = [[1, [1, 1]]];
    $scope.cashella.unlocks[4] = [[1, [1, 1]]];
    $scope.cashella.unlocks[5] = [[1, [1, 1]]];
    $scope.cashella.unlocks[6] = [[1, [1, 1]]];
    $scope.cashella.unlocks[7] = [[1, [1, 1]]];
    $scope.cashella.unlocks[8] = [[1, [1, 1]]];
    $scope.cashella.unlocks[9] = [[1, [1, 1]]];
    $scope.cashella.cashUpgrades = [[1, [1, 1], false]]; // [Cost, [Investment Speed/Investment Profit, Multiplier], false]
    $scope.cashella.angelUpgrades = [[1, [1, 1], false, false]]; // [Cost, [Investment Speed/Investment Profit, Multiplier], false, false]
